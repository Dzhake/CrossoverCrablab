...
namespace Celeste.Mod.CrossoverCrablab.Entities
{
    [CustomEntity("CrossoverCrablab/ElephantRaveController")]
    ...
    public class ElephantRaveController(EntityData data, Vector2 offset) : Entity(data.Position + offset)
    {
        ...
        public override void Awake(Scene scene)
        {
            ...
            if (bg == null)
                throw new ArgumentException("Styleground required for elephant rave was not found!");
            ...
            foreach (Entity e in Scene.Entities)
            {
                switch (e)
                {
                    case CustomDecal d when d.Name == picture_decal_path:
                        picture_decal = d;
                        break;
                    case CustomDecal d when d.Name == ws_decal_path:
                    {
                        ws_decal = d;
                        break;
                    }
                    case InvisibleBarrier b:
                        barrier = b;
                        break;
                    default:
                    {
                        if (e.GetType().FullName == "Celeste.Mod.CollabUtils2.Entities.MiniHeart") heart = e;
                        break;
                    }
                }

                if (picture_decal != null && ws_decal != null && barrier != null && heart != null) break;
            }
        }
